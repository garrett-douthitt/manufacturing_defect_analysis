...
For this project, a dataset from Kaggle.com (citation below) on manufacturing defects at a hypothetical company was taken for analysis. This data was pre-cleaned, so a few null values were manually inserted into the dataset to allow for data validation and cleaning later in the process.

## The goals of this analysis project are to:
1. Identify Cost Drivers: Determine which severity level, defect type, or defect location is the most problematic for this company, and which would be the most efficient to focus on improving.
...
## SQL Query
This dataset only had ~1000 entries, but I treated this project as if it were possible to have much larger datasets to work with. Because of this, I started with a SQL query (run in BigQuery) to consolidate all defects that had the same product ID, severity level, type, and location into a single group and track the occurrences for each group, rather than working with each individual defect. Since there were 100 products in this dataset, this would limit any consolidated dataset to a maximum of 900 entries for further processing. This SQL query is below.
...
product_id
```
Once the consolidated dataset was acquired, there were 851 different defect entries for this project. The data was then loaded into a Jupyter notebook for further cleaning and analysis.
...
1. A null severity value was identified and investigation concluded that the value could be updated to a "critical" severity.
2. A null repair cost value was identified as well. This missing value could not be determined, so approval was given by the project manager to assign the average cost of similar defects (all other defects that were functional, internal, critical defects).

## Exploratory Data Analysis
1. First, occurrence was examined to see if any severity, type, or location of defect occurred significantly more frequently than others.
    - Minor defects were most frequent as expected, but interestingly, critical defects were just below minor, and above moderate.
    - Defect occurrence by location and type had some variance, but none stood out as a majority contributor. 
![Alt Text](images/occurrences.png)
...
## Recommendations
- Minor defects: first determine why minor defects are so costly to repair. This may be due to a standard operating procedure issue or a misunderstanding of how these defects ought to be repaired. It may be possible to bring cost or minor repairs down so that even if they are still frequent, they will not be as costly.
